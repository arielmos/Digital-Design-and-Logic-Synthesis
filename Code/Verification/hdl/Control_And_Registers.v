...
reg StartBit;// saves registers[0][0]
...
always @(posedge  clk) begin: block_filler
  if (rst) begin
    ...
    if (StartBit && Block_Done) begin //start_bit == 1 and block_done
        ena_out<=0;
        case (state)
          0: begin//Im_block
      ...
      else  begin// !(StartBit && Block_Done), if we can't start update the StartBit
          StartBit <= registers[0][0];
      end
    end
end
