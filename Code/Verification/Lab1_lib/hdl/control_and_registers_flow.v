//
// Module Lab1_lib.Control_And_Registers.flow
//
// Created:
//          by - amitnag.UNKNOWN (SHOHAM)
//          at - 15:52:58 12/15/2020
//
// Generated by Mentor Graphics' HDL Designer(TM) 2019.2 (Build 5)
//

`resetall
`timescale 1ns/10ps
module Control_And_Registers #(
   // synopsys template
   // synopsys template
   parameter Amba_Addr_Depth = 20,
   parameter Amba_Word       = 16,
   parameter Data_Depth      = 8    //Bit depth of the pixel; Range - 8,16
   //Bit depth of the pixel; Range - 8,16
)
( 
   // Port Declarations
   // Port Declarations
   input   wire                              clk,         // system clock
   input   wire                              rst,         //  Reset active low
   input   wire                              ena_in,      //  Reset active low
   // Port Declarations
   input   wire    [Amba_Addr_Depth-1:0]     Address,     // APB Address Bus
   input   wire    [Amba_Word-1:0]           data_in,     // APB Read Data Bus
   output  reg                               ena_out,     // when a block is ready
   output  reg                               Image_Done,  //State indicator (Output)
   input   wire                              Block_Done,  // when block is done
   output  reg     [(10*9)-1:0]              params,      //[9:0]//9 params size of 9 bits
   output  reg     [(Data_Depth*72*72)-1:0]  Im_block,    //[(72*72)-1:0],  // Max size of block is 72*72 pixels and size of pixel is DATA_DEPTH
   output  reg     [(Data_Depth*72*72)-1:0]  W_block      //[(72*72)-1:0],  // Max size of block is 72*72 pixels and size of pixel is DATA_DEPTH
);

// Internal Declarations




// ### Please start your Verilog code here ### 
//-------------------------------------------------------
//-------------------registers -----------------------
//-------------------------------------------------------
// Internal Declarations



// Module declarations
reg [Amba_Word-1:0] registers [2**Amba_Addr_Depth-1:0];
reg [19:0] M2 ;
reg [19:0] N2 ;
//-------------------------------------------------------
//-------------------------------------------------------
//----------------------Block Logic ---------------------
//-------------------------------------------------------
reg [18:0] block_i; // max 720*720, number of pixels
reg [12:0] pixel_i;  // max pixels in block is 72*72 = 5184
reg [2:0] param_i; // we have 7 params
reg [1:0] state; // 0 - Im_block, 1 - W_block , 2 - params
reg StartBit;// saves registers[0][0]

/////////////////////////////////////////////////////////////////
// Flowchart reg_proc
always @ (posedge  clk)
begin : reg_proc_flowchart_proc
   if (rst) begin
      registers[0] <=0;
   end
   if (ena_in) begin
      registers[Address] <= data_in;
      if (Address == 4) begin
         M2 <= data_in*data_in;
      end
      if (Address == 2) begin
         N2 <= data_in*data_in;
      end
   end
end

/////////////////////////////////////////////////////////////////
// Flowchart block_filler
always @ (posedge  clk)
begin : block_filler_flowchart_proc
   if (rst) begin
      ena_out<=0;
      block_i <= 0;
      pixel_i <=0;
      param_i <= 0;
      state <= 0;
      Image_Done <= 0;
      StartBit <= 0;
   end
   else if (StartBit && Block_Done) begin
      //start_bit == 1 and block_done
           ena_out<=0;
      case (state)
      0 : begin
         //Im_block
         if (pixel_i >= M2) begin
            //if pixel index >= M^2 finished Im_block
                        pixel_i <=0;
                        state <= 1;
         end
         else
         begin
            Im_block[(pixel_i+1)*Data_Depth-1 -: Data_Depth]<= registers[pixel_i + 10 + block_i*M2][Data_Depth-1:0];
            pixel_i <= pixel_i + 1;
         end
      end
      1 : begin
         //W_block
         if (pixel_i >= M2) begin
            //if pixel index >= M^2 finished Im_block
                          pixel_i <=0;
                          state <= 2;
         end
         else
         begin
            W_block[(pixel_i+1)*Data_Depth-1 -: Data_Depth]<= registers[pixel_i + 10 + N2 + block_i*M2][Data_Depth-1:0];
            pixel_i <= pixel_i + 1;
         end
      end
      default : begin
         // state = 2
         if (param_i >= 7) begin
            // block filler finished
                          Image_Done <= 0;
                          param_i <=0;
                          state <= 0;             
                          ena_out <= 1;
                          block_i <= block_i + 1;
            if (block_i+1 >= (N2)/(M2)) begin
               // end of image
                              Image_Done <= 1;
                              block_i <= 0;
            end
         end
         else
         begin
            case (param_i)
            0 : begin
               params[9:0] <= registers[1];
            end
            1 : begin
               params[29:10] <= M2; //M^2  19:0
               params[39:30] <= registers[4];// M
            end
            2 : begin
               params[49:40] <= registers[5];
            end
            3 : begin
               params[59:50] <= registers[6];
            end
            4 : begin
               params[69:60] <= registers[7];
            end
            5 : begin
               params[79:70] <= registers[8];
            end
            default : begin
               params[89:80] <= registers[9]; //Bmax 5:0
            end
            endcase
            param_i <= param_i + 1;
         end
         // else end
      end
      endcase
   end
   else
   begin
      // !(StartBit && Block_Done), if we can't start update the StartBit
              StartBit <= registers[0][0];
   end
end

endmodule // Control_And_Registers

